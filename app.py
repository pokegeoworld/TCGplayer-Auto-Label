...
st.markdown("""
    ...
    /* REFINED DOWNLOAD BUTTON (Guaranteed Visibility) */
...
# --- 4. THE MASTER PDF CREATOR ---
def create_label_pdf(data, items):
    packet = io.BytesIO()
    can = canvas.Canvas(packet, pagesize=(4*inch, 6*inch))
    
    # Customer Address (14pt Bold)
    ...
    # Packing List Table (Fixing Item List Display)
    ...
    table = Table(table_data, colWidths=[0.4*inch, 2.15*inch, 0.45*inch, 0.5*inch])
...
# --- 5. MASTER DATA EXTRACTION (Jesus Romero Sample Fix) ---
def extract_tcg_data(uploaded_file):
    ...
    try:
        # Targeting specific data from Jesus Romero sample [cite: 14-17, 24, 25]
        data = {
            'buyer_name': lines[1],
            'address': lines[2],
        ...
        # Capture Items from CSV Table Format 
        items = []
...
if profile.get('tier') == 'New':
    ...
    with c3:
        st.markdown('<div class="pricing-card"><p class="tier-name">BASIC</p><p class="big-stat">50</p><p class="label-text">Labels</p></div>', unsafe_allow_html=True)
        st.link_button("Choose Basic", "https://buy.stripe.com/aFafZj9hu7b9dV0f5absc02")
    with c4:
        st.markdown('<div class="pricing-card"><p class="tier-name">PRO</p><p class="big-stat">150</p><p class="label-text">Labels</p></div>', unsafe_allow_html=True)
        st.link_button("Choose Pro", "https://buy.stripe.com/4gM3cx9hu1QP04a5uAbsc01")
    with c5:
        st.markdown('<div class="pricing-card"><p class="tier-name">UNLIMITED</p><p class="big-stat">∞</p><p class="label-text">Labels</p></div>', unsafe_allow_html=True)
...
if uploaded_file:
    ...
    if h_data:
        # CACHE BYTES: Generate PDF bytes once to ensure download stability
        final_pdf_bytes = create_label_pdf(h_data, i_list)
        
        # DOWNLOAD BUTTON: Pre-loaded with bytes
        st.download_button(
