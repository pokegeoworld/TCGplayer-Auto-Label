...
# --- 4. THE DYNAMIC PDF CREATOR (NOW WITH SAVED RETURN ADDRESS) ---
def create_label_pdf(data, items, r_name, r_addr, r_city):
    ...
    # Right Side: DYNAMIC Return Address
    right_x = 4.8 * inch
...
if not profile:
    try:
        ...
        profile = {"id": user.id, "credits": 0, "tier": "None", "return_name": "Poke Geo", "return_address": "36 Michael Anthony Ln", "return_city_zip": "Depew, NY 14043"}
    except: st.error("Profile sync error."); st.stop()

# --- 7. SIDEBAR USERNAME & SETTINGS (SAVING ADDRESS CAPABILITY) ---
...
# Fetch existing or set defaults
rn = st.sidebar.text_input("Return Name", value=profile.get('return_name', 'Poke Geo'))
ra = st.sidebar.text_input("Address Line", value=profile.get('return_address', '36 Michael Anthony Ln'))
rcz = st.sidebar.text_input("City, State Zip", value=profile.get('return_city_zip', 'Depew, NY 14043'))
...
if uploaded_file:
    ...
    try:
        ...
        # Pass saved return address values to the PDF generator
        pdf_bytes = create_label_pdf(data, items, rn, ra, rcz)
        st.download_button(label=f"📥 DOWNLOAD LABEL: {order_no}", data=pdf_bytes, file_name=f"TCGplayer_{order_no}.pdf", mime="application/pdf", use_container_width=True,
