...
st.markdown("""
    ...
    .stDownloadButton > button { background-color: #15803d !important; color: white !important; font-size: 24px !important; height: 80px !important; font-weight: 800 !important; border-radius: 12px !important; border: 2px solid #14532d !important; }
    ...
    .glitch-note-red { color: #FF0000; font-size: 11px; font-weight: 900; text-align: center; margin-top: 15px; border: 2px dashed #FF0000; padding: 8px; border-radius: 8px; white-space: nowrap; }
...
# --- 4. PDF GENERATOR (UPDATED DETAILS LAYOUT) ---
def create_label_pdf(data, items, r_name, r_addr, r_city):
    ...
    # ADJUSTED DETAILS FONT AND STACKED RIGHT COLUMN
    ...
    # Right Column (Incrementally Smaller Font, Stacked)
...
if profile['credits'] == 0 and profile['tier'] == "None":
    ...
    with colB:
        st.markdown('<div class="pricing-card"><p class="tier-name">Starter Pack</p><p class="big-stat">10</p><p class="label-text">Labels</p><p class="small-price">$0.50</p></div>', unsafe_allow_html=True); st.link_button("Buy Starter", "https://buy.stripe.com/28EeVf0KY7b97wC3msbsc03")
    st.markdown('<div class="sub-header">MONTHLY SUBSCRIPTIONS</div>', unsafe_allow_html=True)
    c1, c2, c3 = st.columns(3)
    with c1: st.markdown('<div class="pricing-card"><p class="tier-name">BASIC</p><p class="big-stat">50</p><p class="label-text">Labels</p><p class="small-price">$1.49/mo</p></div>', unsafe_allow_html=True); st.link_button("Choose Basic", "https://buy.stripe.com/aFafZj9hu7b9dV0f5absc02")
    with c2: st.markdown('<div class="pricing-card"><p class="tier-name">PRO</p><p class="big-stat">150</p><p class="label-text">Labels</p><p class="small-price">$1.99/mo</p></div>', unsafe_allow_html=True); st.link_button("Choose Pro", "https://buy.stripe.com/4gM3cx9hu1QP04a5uAbsc01")
    with c3: st.markdown('<div class="pricing-card"><p class="tier-name">UNLIMITED</p><p class="big-stat">∞</p><p class="label-text">Labels</p><p class="small-price">$2.99/mo</p></div>', unsafe_allow_html=True); st.link_button("Choose Unlimited", "https://buy.stripe.com/28E9AV1P2anlaIO8GMbsc00")
    st.stop()
...
if uploaded_file:
    ...
    try:
        ...
        pdf_bytes = create_label_pdf(data, items, rn, ra, rcz)
