...
if "user" not in st.session_state:
    ...
    if signup_btn:
        ...
        except Exception: st.sidebar.error("Signup failed. Check trigger timing (AFTER).")
    st.stop()
...
if not profile:
    st.markdown('<p class="hero-title">TCGplayer Auto Label Creator</p>', unsafe_allow_html=True)
    st.error("Account sync pending. Please refresh the page.")
    ...
    if profile.get('credits', 0) > 0:
        ...
        if items:
            ...
            try:
                ...
                st.success(f"Formatted {len(items)} items.")
