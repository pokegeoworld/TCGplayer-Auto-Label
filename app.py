...
def create_label_pdf(data, items):
    ...
    for item in items:
        ...
        total_qty += int(item['qty']); grand_total += float(item['total'].replace('$', '')); y_pos -= 0.07 * inch
...
def extract_tcg_data(uploaded_file):
    ...
    try:
        ...
        data = {
            'buyer_name': lines[ship_idx + 1], 'address': lines[ship_idx + 2], 'city_state_zip': lines[ship_idx + 3],
...
# --- 8. SMART PRICING GATE (FIXED) ---
# Only show pricing if user is brand new and has not selected any tier yet
